...
final class WebCoordinator: NSObject {
    ...
    private let cookieJar = "stagebuilder_cookies"
    
    func loadURL(_ url: URL, in webView: WKWebView) {
        print("🎭 [StageBuilder] Load: \(url.absoluteString)")
        redirectCount = 0
...
extension WebCoordinator: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
        if let current = webView.url { checkpoint = current; print("✅ [StageBuilder] Commit: \(current.absoluteString)") }
    }
